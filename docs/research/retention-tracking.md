...
| 2 | Daily game → Day inspector, beside the **published day** | Daily game completion · that day · per language · shown against the same weekday in the prior 4 weeks | Whether that day's crossword or Guessword was mis-scheduled or too hard, and whether to publish a correction |
...
**Could not verify:** every widely circulated Guessword and NYT Games engagement figure (annual plays, daily players, "games subscribers retain best"). NYT-owned domains were unreachable from this run and the company's own 2024 results release discloses no games metric, so none of those numbers appear above as fact. Also dropped: a frequently repeated claim that Streak Freeze "reduced churn 21%", which traces only to secondary marketing write-ups with no primary Duolingo source.
