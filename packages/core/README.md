...
Base classes for the "Durable Object aggregate + D1 projection" pattern described in
...
Copy `test/wrangler.jsonc`, `test/setup.ts`, `test/env.d.ts` and `vitest.config.ts` into your
Worker package, point them at your migrations, and write tests against real bindings:
